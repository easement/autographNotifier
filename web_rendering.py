...
def _nav_html(active: str) -> str:
    ...
    return (
        '<nav class="site-nav">'
        + _link("index.html", "By Date", "date")
        + _link("by-store.html", "By Store", "store")
        + '</nav>'
    )
...
_NAV_CSS = """
  /* ── Site nav ── */
  .site-nav {
    display: flex; gap: 0; margin-left: 16px; flex-shrink: 0;
  }
  .nav-link {
    font-family: var(--font-display); font-size: 0.62rem;
    letter-spacing: 0.18em; text-transform: uppercase; font-weight: 600;
    color: var(--ink-3); text-decoration: none;
    border: 1.5px solid var(--border); padding: 4px 14px;
    margin-left: -1px;
    transition: background 0.15s, color 0.15s, border-color 0.15s;
    background: var(--paper);
  }
  .nav-link:hover { color: var(--ink); border-color: var(--ink); z-index: 1; }
  .nav-link--active {
    color: var(--paper); background: var(--ink); border-color: var(--ink); z-index: 1;
  }
"""


def generate_html(listings: list[WebListingViewModel], page: str = "date") -> str:
    today_iso = date.today().isoformat()
    generated_at = datetime.now().strftime("%B %d, %Y at %I:%M %p")
    total = len(listings)
    listings_json = json.dumps([vars(item) for item in listings], ensure_ascii=False)
    nav_html = _nav_html(page)
...
def generate_html_by_shop(listings: list[WebListingViewModel]) -> str:
    generated_at = datetime.now().strftime("%B %d, %Y at %I:%M %p")
    total = len(listings)
    listings_json = json.dumps([vars(item) for item in listings], ensure_ascii=False)
    nav_html = _nav_html("store")

    sorted_listings = sorted(listings, key=lambda x: x.shop.lower())

    shop_sections_html = ""
    for shop_name, group in groupby(sorted_listings, key=lambda x: x.shop):
        group_list = list(group)
        ...
        shop_sections_html += f"""
  <div class="date-section shop-section" data-shop="{_esc(shop_name)}">
    <div class="date-heading">
      <div class="date-label">{_esc(shop_name)}</div>
      <div class="date-count">{len(group_list)} listing{"s" if len(group_list) != 1 else ""}</div>
    </div>
    <table class="listings-table">
      <thead>
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>By Store — Autograph Notifier</title>
<link rel="icon" type="image/svg+xml" href="favicon.svg">
<script async src="https://www.googletagmanager.com/gtag/js?id=G-TBYS6FEB3R"></script>
<script>
  window.dataLayer = window.dataLayer || [];
...
<main class="main" id="mainContent">
{shop_sections_html if shop_sections_html else '<div class="empty-state">No listings found.</div>'}
  <div id="flatView" style="display:none">
    <table class="listings-table">
      <thead>
        <tr>
